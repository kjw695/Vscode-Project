        ...
        >
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-4 ${isDarkMode ? 'text-gray-200' : 'text-gray-700'}`}>
                ...
                <p className="md:col-span-2 text-lg font-semibold"><strong>월 순이익:</strong> {monthlyProfit.netProfit.toLocaleString()} 원</p>
                        ...
                        {activeContentTab === 'adminSettings' && isAuthReady && (
                            <div className={`p-6 rounded-lg shadow-md w-full max-w-4xl mb-6 ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
                                <h2 className={`text-2xl font-bold mb-4 ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>관리자 설정</h2>
                                
                                {/* 계정 관리 섹션 */}
                                <div className="mb-6">
    <h3 className={`text-xl font-semibold mb-3 ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>계정 관리</h3>
        ...
        auth.currentUser?.isAnonymous ? (
            <div className="text-center mb-4">
                ...
                <div className="flex flex-col items-center space-y-3">
                    <button
                        onClick={() => handleLinkAccount(kakaoProvider)}
                        className="bg-[#FEE500] text-[#191919] font-bold py-2 px-4 rounded-md shadow-md transition duration-150 ease-in-out w-64 text-center"
                    >
                        카카오 계정으로 데이터 저장
                    </button>
                    <button
                        onClick={() => handleLinkAccount(naverProvider)}
                        className="bg-[#03C75A] text-white font-bold py-2 px-4 rounded-md shadow-md transition duration-150 ease-in-out w-64 text-center"
                    >
                        네이버 계정으로 데이터 저장
                    </button>
                    <button
                        onClick={() => handleLinkAccount(googleProvider)}
                        className="bg-white hover:bg-gray-100 text-gray-800 border border-gray-300 font-bold py-2 px-4 rounded-md shadow-md transition duration-150 ease-in-out w-64 flex items-center justify-center"
                    >
            ...
            <div className="text-center mb-4">
                <p className={`text-sm mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                    {auth.currentUser?.photoURL && <img src={auth.currentUser.photoURL} alt="profile" className="w-12 h-12 rounded-full mx-auto mb-2" />}
                    {auth.currentUser?.displayName || auth.currentUser?.email} 계정으로 로그인되어 있습니다.
                </p>
                <button
                ...
                로그아웃
                                ...
                                {/* ... 나머지 관리자 설정 섹션 ... */}
                                <div className="mb-6">
                                    <h3 className={`text-xl font-semibold mb-3 ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>즐겨찾는 단가 설정</h3>
                                    <div className="mb-4">
                                ...
                                <div className="mb-6">
                                    <h3 className={`text-xl font-semibold mb-3 ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>월별 집계 기간 설정</h3>
                                ...
                                <div className="mb-6">
                                    <h3 className={`text-xl font-semibold mb-3 ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>데이터 백업 및 복원 (JSON)</h3>
                                    <div className="mb-4">
                                        <button
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`block w-full text-sm ${isDarkMode ? 'text-gray-100 bg-gray-700 border-gray-600' : 'text-gray-700 bg-white border-gray-300'} border rounded-md cursor-pointer focus:outline-none focus:ring-blue-500 focus:border-blue-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold ${isDarkMode ? 'file:bg-blue-600 file:text-white' : 'file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100'}`}
                                        />
                                        <p className={`text-sm mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                                            선택한 JSON 파일의 데이터가 현재 계정에 추가됩니다.
                                        </p>
                                    </div>
                                </div>

                                <div className="mb-6">
                                    <h3 className={`text-xl font-semibold mb-3 ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>데이터 내보내기/가져오기 (CSV)</h3>
                                    <div className="mb-4">
                                        <button
                                            onClick={handleExportCsv}
                                            className="w-full bg-purple-600 text-white py-2 px-4 rounded-md hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 transition duration-150 ease-in-out flex items-center justify-center"
                                        >
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`block w-full text-sm ${isDarkMode ? 'text-gray-100 bg-gray-700 border-gray-600' : 'text-gray-700 bg-white border-gray-300'} border rounded-md cursor-pointer focus:outline-none focus:ring-purple-500 focus:border-purple-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold ${isDarkMode ? 'file:bg-purple-600 file:text-white' : 'file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100'}`}
                                        />
                                        <p className={`text-sm mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                                            선택한 CSV 파일의 데이터가 현재 계정에 추가됩니다.
                                        </p>
                                    </div>
            ...
            {isAuthReady && ( // 인증 준비가 되면 하단 바 표시
                <div className={`fixed bottom-0 left-0 right-0 w-full ${isDarkMode ? 'bg-gray-800 border-t border-gray-700' : 'bg-white border-t border-gray-200'} shadow-lg flex justify-around py-2 px-4 pb-[env(safe-area-inset-bottom)]`}>
                    ...
                    <button
                        ...
                        onClick={() => { setSelectedMainTab('more'); setActiveContentTab('adminSettings'); }}
                    >
