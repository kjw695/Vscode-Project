...
const EntriesList = ({ entries, summary, handleEdit, handleDelete, isDarkMode }) => {
  ...
  return (
    <div className="w-full space-y-6">
      {/* 수익 / 지출 요약 카드 */}
      <div className="p-4 rounded-lg bg-gray-100 dark:bg-gray-800 shadow-sm">
        {summary.totalRevenue > 0 && (
          <div className="flex justify-between items-center py-1">
            <span className="font-semibold text-gray-600 dark:text-gray-300">수익</span>
            <span className="font-bold text-red-500 text-lg">{summary.totalRevenue.toLocaleString()}</span>
          </div>
        )}
        ...
        {summary.totalExpenses > 0 && (
          <div className="flex justify-between items-center py-1">
            ...
            <span className="font-bold text-blue-500 text-lg">{summary.totalExpenses.toLocaleString()}</span>
          </div>
        )}
      </div>
            ...
            {!isCollapsed && (
              <div className={`rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-md`}>
                {groupedEntries[month].map((entry, index) => {
                  const dailyRevenue = (entry.unitPrice * entry.deliveryCount) + (entry.unitPrice * entry.returnCount) + (entry.deliveryInterruptionAmount || 0) + ((entry.freshBagCount || 0) * 100);
                    ...
                    <div key={entry.id} className={`${index < groupedEntries[month].length - 1 ? (isDarkMode ? 'border-b border-gray-700' : 'border-b border-gray-200') : ''}`}>
                      <div className="p-4 flex flex-col">
                        ...
                        <div className="flex justify-between items-end mt-2">
                          ...
                          <p className={`font-bold text-xl ${netProfit >= 0 ? 'text-red-500' : 'text-blue-500'}`}>
                            {netProfit >= 0 ? '+' : ''}{netProfit.toLocaleString()}
                          </p>
                        </div>
                      </div>
                      
                      {/* 👇 핵심 수정: 상세 보기 전체를 하나의 부모 태그(<>)로 감쌌습니다. */}
                      {expandedId === entry.id && (
                        <>
